...
public class UInt64TypeTests
{
    ...
    public void ReadValue_SingleValue_ReturnsCorrectValue()
    {
        ...
        var reader = UInt64Type.Instance;
    ...
    {
        // Arrange: ulong.MaxValue in little-endian bytes
    ...
    {
        // Arrange: ulong.MinValue (0) in little-endian bytes
    ...
    {
        // Arrange
        var values = Enumerable.Range(1, count).Select(x => (ulong)x).ToArray();
        ...
        var sequence = new ReadOnlySequence<byte>(bytes);
        var reader = UInt64Type.Instance;
        var destination = new ulong[count];
    ...
    public void WriteValue_SingleValue_ProducesCorrectBytes()
    {
        ...
        var typeHandler = UInt64Type.Instance;

        // Act
        typeHandler.WriteValue(writer, 42ul);
    ...
    public void WriteValue_MaxValue_ProducesCorrectBytes()
    {
        ...
        // Act
        typeHandler.WriteValue(writer, ulong.MaxValue);
    ...
    public void WriteValue_MinValue_ProducesCorrectBytes()
    {
        ...
        // Act
        typeHandler.WriteValue(writer, ulong.MinValue);

        // Assert
        var result = writer.WrittenSpan.ToArray();
        Assert.Equal([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], result);
    }
    ...
    [InlineData(18446744073709551614ul)] // MaxValue - 1
    public void RoundTrip_SingleValue_PreservesValue(ulong value)
    {
        // Arrange
        var typeHandler = UInt64Type.Instance;
        var writer = new ArrayBufferWriter<byte>();
    ...
    public void RoundTrip_LargeArray_PreservesAllValues()
    {
        // Arrange
        var typeHandler = UInt64Type.Instance;
        var values = new ulong[1000];
        var random = new Random(42);
        for (var i = 0; i < values.Length; i++)
        {
            values[i] = ((ulong)random.Next() << 32) | (uint)random.Next();
        }
        ...
        // Act - Read
        var sequence = new ReadOnlySequence<byte>(buffer);
        var destination = new ulong[values.Length];
    ...
    public void Properties_ReturnCorrectValues()
    {
        ...
        // Assert
        Assert.Equal(0x04, typeHandler.ProtocolCode);
        Assert.Equal("UInt64", typeHandler.TypeName);
        Assert.True(typeHandler.IsFixedLength);
        Assert.Equal(8, typeHandler.FixedByteLength);
        Assert.Equal(typeof(ulong), typeHandler.ClrType);
    }
    ...
    {
        // Arrange & Act
        var typeHandler = new UInt64Type();
    ...
    public void SimdCapabilities_CustomInstance_UsesProvidedCapabilities()
    {
        ...
        // Act
        var typeHandler = new UInt64Type(customCapabilities);
        ...
        bool sse2, bool ssse3, bool avx2, bool avx512)
    {
        ...
        var values = Enumerable.Range(1, 100).Select(x => (ulong)x).ToArray();
        var writer = new ArrayBufferWriter<byte>();
