...
public class UInt32TypeSimdTests(ITestOutputHelper output)
{
    [Theory]
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        var testSizes = SimdPathTestHelper.GetTestSizesForType(typeof(uint));
        
        foreach (var size in testSizes)
        {
            output.WriteLine($"  Size: {size}");
            
            var expectedValues = SimdPathTestHelper.GenerateTestData<uint>(size);
            
            // Serialize the data
            var writer = new ArrayBufferWriter<byte>();
            foreach (var value in expectedValues)
            {
                UInt32Type.Instance.WriteValue(writer, value);
            }
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        foreach (var size in testSizes)
        {
            ...
            var values = SimdPathTestHelper.GenerateTestData<uint>(size);
            ...
            var scalarHandler = new UInt32Type(scalarCapabilities);
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        var testSizes = new[] { 5, 9, 17 }; // Odd sizes to ensure partial vectors
        ...
        foreach (var size in testSizes)
        {
            ...
            // Create fragmented sequence (each uint in separate segment)
            ...
            {
                // Create a fragmented sequence with each uint in a separate segment
                var firstSegment = new BufferSegment(new Memory<byte>(bytes, 0, sizeof(uint)));
                var lastSegment = firstSegment;
                
                for (var i = 1; i < size; i++)
                {
                    var nextSegment = new BufferSegment(
                        new Memory<byte>(bytes, i * sizeof(uint), sizeof(uint)));
                    lastSegment.Append(nextSegment);
                    lastSegment = nextSegment;
                }
                
                sequence = new ReadOnlySequence<byte>(firstSegment, 0, lastSegment, sizeof(uint));
            }
    ...
    public void ReadValues_VerifySimdPathSelection()
    {
        ...
        var testData = SimdPathTestHelper.GenerateTestData<uint>(20);
        ...
        if (DefaultSimdCapabilities.Instance.IsAvx2Supported)
        {
            ...
            var avx2Handler = new UInt32Type(
        ...
        if (DefaultSimdCapabilities.Instance.IsSse2Supported)
        {
            ...
            var sse2Handler = new UInt32Type(
        ...
        // Test scalar path
        {
            ...
            var scalarHandler = new UInt32Type(
    ...
    [InlineData(15, "Just below AVX512F boundary")]
    [InlineData(16, "Exactly AVX512F boundary")]
    [InlineData(17, "Just above AVX512F boundary")]
