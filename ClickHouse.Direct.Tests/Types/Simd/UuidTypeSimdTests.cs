...
public class UuidTypeSimdTests(ITestOutputHelper output)
{
    [Theory]
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        var testSizes = SimdPathTestHelper.GetTestSizesForType(typeof(Guid));
        
        foreach (var size in testSizes)
        {
            output.WriteLine($"  Size: {size}");
            
            var expectedValues = SimdPathTestHelper.GenerateTestData<Guid>(size);
            
            // Serialize the data
            var writer = new ArrayBufferWriter<byte>();
            foreach (var value in expectedValues)
            {
                UuidType.Instance.WriteValue(writer, value);
            }
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        foreach (var size in testSizes)
        {
            ...
            var values = SimdPathTestHelper.GenerateTestData<Guid>(size);
            ...
            var scalarHandler = new UuidType(scalarCapabilities);
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, string description)
    {
        ...
        var testSizes = new[] { 2, 3, 5 }; // Odd sizes to ensure partial vectors
        ...
        foreach (var size in testSizes)
        {
            ...
            // Create fragmented sequence (each UUID in separate segment)
            ...
            {
                // Create a fragmented sequence with each UUID in a separate segment
                var firstSegment = new BufferSegment(new Memory<byte>(bytes, 0, 16));
                var lastSegment = firstSegment;
                
                for (var i = 1; i < size; i++)
                {
                    var nextSegment = new BufferSegment(
                        new Memory<byte>(bytes, i * 16, 16));
                    lastSegment.Append(nextSegment);
                    lastSegment = nextSegment;
                }
                
                sequence = new ReadOnlySequence<byte>(firstSegment, 0, lastSegment, 16);
            }
    ...
    public void ReadValues_VerifySimdPathSelection()
    {
        ...
        var testData = SimdPathTestHelper.GenerateTestData<Guid>(10);
        ...
        // Test SSE2 path (processes 1 value at once - no SIMD benefit for single UUID)
        if (DefaultSimdCapabilities.Instance.IsSse2Supported)
        {
            output.WriteLine("Testing SSE2 path");
            var sse2Handler = new UuidType(
        ...
        // Test scalar path
        {
            ...
            var scalarHandler = new UuidType(
    ...
    [Theory]
    [InlineData(1, "Single UUID")]
    [InlineData(2, "Exactly AVX2 boundary")]
    [InlineData(3, "Just above AVX2 boundary")]
    [InlineData(4, "Exactly AVX512F boundary")]
    [InlineData(5, "Just above AVX512F boundary")]
