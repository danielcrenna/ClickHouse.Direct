...
public class Int16TypeSimdTests(ITestOutputHelper output)
{
    [Theory]
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, bool avx512Bw, string description)
    {
        ...
        var testSizes = SimdPathTestHelper.GetTestSizesForType(typeof(short));
        
        foreach (var size in testSizes)
        {
            output.WriteLine($"  Size: {size}");
            
            var expectedValues = SimdPathTestHelper.GenerateTestData<short>(size);
            
            // Serialize the data
            var writer = new ArrayBufferWriter<byte>();
            foreach (var value in expectedValues)
            {
                Int16Type.Instance.WriteValue(writer, value);
            }
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, bool avx512Bw, string description)
    {
        ...
        foreach (var size in testSizes)
        {
            ...
            var values = SimdPathTestHelper.GenerateTestData<short>(size);
            ...
            var scalarHandler = new Int16Type(scalarCapabilities);
        ...
        bool sse2, bool ssse3, bool avx, bool avx2, bool avx512F, bool avx512Bw, string description)
    {
        ...
        foreach (var size in testSizes)
        {
            ...
            // Create fragmented sequence (each short in separate segment)
            ...
            {
                // Create a fragmented sequence with each short in a separate segment
                var firstSegment = new BufferSegment(new Memory<byte>(bytes, 0, sizeof(short)));
                var lastSegment = firstSegment;
                
                for (var i = 1; i < size; i++)
                {
                    var nextSegment = new BufferSegment(
                        new Memory<byte>(bytes, i * sizeof(short), sizeof(short)));
                    lastSegment.Append(nextSegment);
                    lastSegment = nextSegment;
                }
                
                sequence = new ReadOnlySequence<byte>(firstSegment, 0, lastSegment, sizeof(short));
            }
    ...
    public void ReadValues_VerifySimdPathSelection()
    {
        ...
        var testData = SimdPathTestHelper.GenerateTestData<short>(100);
        ...
        // Test scalar path
        {
            ...
            var scalarHandler = new Int16Type(
