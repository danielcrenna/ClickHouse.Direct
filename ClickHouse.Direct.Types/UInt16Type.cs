...
/// <summary>
/// ClickHouse UInt16 type handler with SIMD-optimized serialization/deserialization.
///
/// https://clickhouse.com/docs/sql-reference/data-types/int-uint
/// 
/// The UInt16 data type represents unsigned 16-bit integers with a value range from 0 to 65,535.
/// Values are stored in little-endian byte order for RowBinary format (2 bytes per value).
/// 
/// This implementation provides:
/// - AVX512F: Processes 32 UInt16 values simultaneously (64 bytes) using 512-bit vectors
/// - AVX2: Processes 16 UInt16 values simultaneously (32 bytes) using 256-bit vectors
/// - SSE2: Processes 8 UInt16 values simultaneously (16 bytes) using 128-bit vectors
...
/// </summary>
public sealed class UInt16Type(ISimdCapabilities simdCapabilities) : BaseClickHouseType<ushort>
{
    ...
    public UInt16Type() : this(DefaultSimdCapabilities.Instance) { }

    public override byte ProtocolCode => 0x02;
    public override string TypeName => "UInt16";
    public override bool IsFixedLength => true;
    public override int FixedByteLength => sizeof(ushort);

    public override ushort ReadValue(ref ReadOnlySequence<byte> sequence, out int bytesConsumed)
    {
        ...
        if (sequence.First.Length >= size)
        {
            ...
            var value = BinaryPrimitives.ReadUInt16LittleEndian(sequence.First.Span);
    ...
    public override int ReadValues(ref ReadOnlySequence<byte> sequence, Span<ushort> destination, out int bytesConsumed)
    {
    ...
    public override void WriteValue(IBufferWriter<byte> writer, ushort value)
    {
    ...
    public override void WriteValues(IBufferWriter<byte> writer, ReadOnlySpan<ushort> values)
    {
        if (values.IsEmpty)
            return;
    ...
    private static void ReadValuesScalar(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
        if (BitConverter.IsLittleEndian)
        {
            ...
            var sourceBytes = source[..(destination.Length * sizeof(ushort))];
    ...
    private unsafe void ReadValuesSse2(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
        var i = 0;
        const int vectorSize = 8; // 8 uint16s per SSE2 vector
        ...
        if (BitConverter.IsLittleEndian)
        {
            ...
            fixed (ushort* destPtr = destination)
            {
                ...
                {
                    var vec = Sse2.LoadVector128((ushort*)(srcPtr + i * sizeof(ushort)));
        ...
        else
        {
            ...
            fixed (byte* srcPtr = source)
            fixed (ushort* destPtr = destination)
            fixed (ushort* tempPtr = temp)
            {
                for (; i + vectorSize <= destination.Length; i += vectorSize)
                {
                    var vec = Sse2.LoadVector128((ushort*)(srcPtr + i * sizeof(ushort)));
                    // Reverse bytes within each uint16 for endianness
                    if (SimdCapabilities.IsSsse3Supported)
                    {
                        vec = Ssse3.Shuffle(vec.AsByte(), ShuffleMaskSse2).AsUInt16();
                    }
                    else
                    {
        ...
        {
            ReadValuesScalar(source[(i * sizeof(ushort))..], destination[i..]);
        }
    }
    
    private unsafe void ReadValuesAvx2(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
        var i = 0;
        const int vectorSize = 16; // 16 uint16s per AVX2 vector
        ...
        if (BitConverter.IsLittleEndian)
        {
            ...
            fixed (ushort* destPtr = destination)
            {
                for (; i + vectorSize <= destination.Length; i += vectorSize)
                {
                    var vec = Avx.LoadVector256((ushort*)(srcPtr + i * sizeof(ushort)));
        ...
        {
            // Handle endianness conversion with SIMD
            Span<ushort> temp = stackalloc ushort[16];
            ...
            fixed (ushort* tempPtr = temp)
            {
                for (; i + vectorSize <= destination.Length; i += vectorSize)
                {
                    ...
                    {
                        // Reverse bytes within each uint16 for endianness
                        vec = Avx2.Shuffle(vec.AsByte(), ShuffleMaskAvx2).AsUInt16();
                    }
                    else
                    {
                        // Fallback
        ...
        {
            ReadValuesSse2(source[(i * sizeof(ushort))..], destination[i..]);
        }
        else
        {
            ReadValuesScalar(source[(i * sizeof(ushort))..], destination[i..]);
    ...
    private unsafe void ReadValuesAvx512(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
        var i = 0;
        const int vectorSize = 32; // 32 uint16s per AVX512 vector
        ...
        if (BitConverter.IsLittleEndian)
        {
            ...
            fixed (ushort* destPtr = destination)
            {
                for (; i + vectorSize <= destination.Length; i += vectorSize)
                {
                    var vec = Avx512F.LoadVector512((ushort*)(srcPtr + i * sizeof(ushort)));
        ...
        else
        {
            ...
            Span<ushort> temp = stackalloc ushort[32];
            ...
            fixed (ushort* tempPtr = temp)
            {
                ...
                {
                    var vec = Avx512F.LoadVector512((ushort*)(srcPtr + i * sizeof(ushort)));
                    // For AVX512BW, we could use byte shuffle, but for now use uint16 operations
        ...
        {
            ReadValuesAvx2(source[(i * sizeof(ushort))..], destination[i..]);
        }
    ...
    private static void WriteValuesScalar(ReadOnlySpan<ushort> values, Span<byte> destination)
    {
        if (BitConverter.IsLittleEndian)
        {
            // Fast path: can directly copy memory on little-endian systems
    ...
    private unsafe void WriteValuesSse2(ReadOnlySpan<ushort> values, Span<byte> destination)
    {
        var i = 0;
        const int vectorSize = 8; // 8 uint16s per SSE2 vector
        ...
        if (i < values.Length)
        {
            WriteValuesScalar(values[i..], destination[(i * sizeof(ushort))..]); 
        }
    }
    
    private unsafe void WriteValuesAvx2(ReadOnlySpan<ushort> values, Span<byte> destination)
    {
        var i = 0;
        const int vectorSize = 16; // 16 uint16s per AVX2 vector
        ...
        if (BitConverter.IsLittleEndian)
        {
            ...
            fixed (byte* destPtr = destination)
            {
                for (; i + vectorSize <= values.Length; i += vectorSize)
                {
                    ...
                    Avx.Store((ushort*)(destPtr + i * sizeof(ushort)), vec);
                }
            }
        }
        else
        ...
        {
            WriteValuesSse2(values[i..], destination[(i * sizeof(ushort))..]); 
        }
        else
        {
            WriteValuesScalar(values[i..], destination[(i * sizeof(ushort))..]); 
    ...
    private unsafe void WriteValuesAvx512(ReadOnlySpan<ushort> values, Span<byte> destination)
    {
        var i = 0;
        const int vectorSize = 32; // 32 uint16s per AVX512 vector
        ...
        if (BitConverter.IsLittleEndian)
        {
            ...
            fixed (byte* destPtr = destination)
            {
                for (; i + vectorSize <= values.Length; i += vectorSize)
                {
                    ...
                    Avx512F.Store((ushort*)(destPtr + i * sizeof(ushort)), vec);
                }
            }
        }
        else
        ...
        {
            WriteValuesAvx2(values[i..], destination[(i * sizeof(ushort))..]); 
        }
