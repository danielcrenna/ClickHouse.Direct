...
/// <summary>
/// ClickHouse Decimal32 type handler with SIMD-optimized serialization/deserialization.
///
/// https://clickhouse.com/docs/sql-reference/data-types/decimal
/// 
/// Decimal32 stores fixed-point decimal numbers with up to 9 digits total.
/// The value is stored as a 32-bit signed integer representing the unscaled value.
/// Scale parameter (0-9) determines the number of decimal places.
/// 
/// This implementation provides:
/// - AVX512F: Processes 16 decimals simultaneously (64 bytes) using 512-bit vectors  
/// - AVX2: Processes 8 decimals simultaneously (32 bytes) using 256-bit vectors
/// - SSE2: Processes 4 decimals simultaneously (16 bytes) using 128-bit vectors
...
/// </summary>
public sealed class Decimal32Type : BaseClickHouseType<decimal>
{
    public const byte MaxPrecision = 9;
    ...
    public Decimal32Type(byte precision = MaxPrecision, byte scale = DefaultScale, ISimdCapabilities? simdCapabilities = null)
    {
    ...
    public static Decimal32Type CreateWithScale(byte scale) => new(MaxPrecision, scale);
    public static readonly Decimal32Type Instance = new();

    public override byte ProtocolCode => 0x42;
    public override string TypeName => $"Decimal32({Precision},{Scale})";
    ...
    public override decimal ReadValue(ref ReadOnlySequence<byte> sequence, out int bytesConsumed)
    {
        if (sequence.Length < sizeof(int))
            throw new InvalidOperationException($"Insufficient data to read Decimal32. Expected {sizeof(int)} bytes, got {sequence.Length}");
    ...
    public override void WriteValue(IBufferWriter<byte> writer, decimal value)
    {
        ...
        var unscaledValue = (int)(value * _scaleFactor);
        
        if (BitConverter.IsLittleEndian)
            BinaryPrimitives.WriteInt32LittleEndian(span, unscaledValue);
        else
            BinaryPrimitives.WriteInt32BigEndian(span, unscaledValue);
    ...
    public override void WriteValues(IBufferWriter<byte> writer, ReadOnlySpan<decimal> values)
    {
        ...
        // Convert decimal to int unscaled values first
        ...
        while (remaining > 0)
        {
            ...
            // Convert batch of decimal to int
            for (var i = 0; i < batchSize; i++)
            {
                currentRawValues[i] = (int)(values[srcIndex++] * _scaleFactor);
            }
