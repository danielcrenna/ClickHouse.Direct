...
public class Int8TypeIntegrationTests(ClickHouseContainerFixture fixture, ITestOutputHelper output)
    : TypeIntegrationTestBase(fixture, output)
{
    [Fact]
    public async Task InsertAndSelect_UsingRowBinary_ShouldRoundTrip()
    {
        var tableName = GetSanitizedTableName("test_int8_rowbinary");
        ...
        Output.WriteLine($"Inserting {testValues.Length} Int8 values using RowBinary format");
        ...
        var actualValues = new List<sbyte>();
        
        foreach (var b in expectedSorted)
        {
            var value = Int8Type.Instance.ReadValue(ref sequence, out _);
    ...
    {
        var tableName = GetSanitizedTableName("test_int8_bulk");
        await Transport.ExecuteNonQueryAsync($"DROP TABLE IF EXISTS {tableName}");
        await Transport.ExecuteNonQueryAsync($"""
            CREATE TABLE {tableName} (
                value Int8
        ...
        var values = new sbyte[valueCount];
        ...
        Output.WriteLine($"Inserting {valueCount} random Int8 values");
        ...
        var actualMin = sbyte.Parse(minStr);
        var actualMax = sbyte.Parse(maxStr);
    ...
    [Fact]
    public async Task NullableInt8_RowBinary_HandlesNullsCorrectly()
    {
        var tableName = GetSanitizedTableName("test_nullable_int8");
        await Transport.ExecuteNonQueryAsync($"DROP TABLE IF EXISTS {tableName}");
        await Transport.ExecuteNonQueryAsync($"""
            CREATE TABLE {tableName} (
                id Int32,
                nullable_value Nullable(Int8)
            ) ENGINE = Memory
            """);
        ...
        foreach (var item in testData)
        {
            ...
            if (item.value.HasValue)
            {
                span[0] = 0;
                writer.Advance(1);
                Int8Type.Instance.WriteValue(writer, item.value.Value);
            }
            else
            {
                span[0] = 1;
    ...
    public async Task Performance_BulkOperations_DifferentSizes()
    {
        ...
        {
            var tableName = GetSanitizedTableName($"test_int8_perf_{size}");
            await Transport.ExecuteNonQueryAsync($"DROP TABLE IF EXISTS {tableName}");
            await Transport.ExecuteNonQueryAsync($"""
                CREATE TABLE {tableName} (
                    value Int8
            ...
            var values = new sbyte[size];
            for (var i = 0; i < size; i++)
                values[i] = (sbyte)(i % 128 - 64); // Predictable pattern
            ...
            sw.Restart();
            var sequence = await QueryRowBinaryDataAsync($"SELECT value FROM {tableName}");
            sw.Stop();
            
            Output.WriteLine($"Selected {size} values in {sw.Elapsed.TotalMilliseconds:F2}ms");
            
            sw.Restart();
            var readValues = new List<sbyte>(size);
            for (var i = 0; i < size; i++)
            {
                var value = Int8Type.Instance.ReadValue(ref sequence, out _);
            ...
            Assert.Equal(size, readValues.Count);
            await Transport.ExecuteNonQueryAsync($"DROP TABLE {tableName}");
        }
    }
}